...
async function loadSettings() {
  const settings = await chrome.storage.sync.get(['provider', 'model', 'apiKey', 'language', 'mode', 'theme', 'aspectRatio']);
  ...
  aspectRatioSelect.value = currentSettings.aspectRatio || '9:16';
...
function applyTheme(theme) {
  ...
  themeToggle.title = normalizedTheme === 'dark' ? '切换日间模式' : '切换夜间模式';
...
async function loadHistory() {
  ...
  historyList.innerHTML = recentHistory.map((item, index) => {
    ...
    const aspectText = formatAspectRatioLabel(item.aspectRatio || '9:16');
    const aspectIcon = `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="4" y="5" width="16" height="14" rx="2"></rect><path d="M8 9h8M8 15h8"></path></svg>`;
    ...
    return `
      <div class="history-item" data-index="${realIndex}">
        ...
        <div class="history-item-footer">
          ...
          <span class="history-mode-tag history-aspect-tag">${aspectIcon} ${aspectText}</span>
        </div>
      </div>
    `;
  }).join('');
...
themeToggle.addEventListener('click', async () => {
  ...
  await chrome.storage.sync.set({ theme: currentSettings.theme });
});
